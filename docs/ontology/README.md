...
Scope: Claude Code, Codex, Cursor, GitHub Copilot, Google
...
| Concept | Physical source and scope | Effective semantics |
|---|---|---|
| Settings | `~/.pi/agent/settings.json`; trusted project `.pi/settings.json`; CLI flags; `PI_CODING_AGENT_DIR` relocates the whole agent config directory | Project settings deep-merge over global. Explicit CLI resource flags are invocation-specific. |
| Instructions | `~/.pi/agent/AGENTS.md`; ancestor/current `AGENTS.md` or `CLAUDE.md` (an `AGENTS.override.md` in the same directory replaces both for that directory); project `.pi/SYSTEM.md` then global `~/.pi/agent/SYSTEM.md`; corresponding `APPEND_SYSTEM.md` | System file replaces base prompt; append file appends. Context files load by discovery, but contradictory text has no enforcement precedence. |
| Skills | `~/.pi/agent/skills`, `~/.agents/skills`; trusted project/ancestor `.pi/skills`, `.agents/skills` (`.agents/skills` resolves through cwd ancestors up to the git repo root); packages; settings and CLI resources | `--no-skills` disables discovery, not explicitly supplied skills. Root-level `.md` files with valid frontmatter are individual skills in `~/.pi/agent/skills` and `.pi/skills`, but not in `~/.agents/skills`. Duplicate-name resolution is `UNKNOWN` until source/version proof. |
| Extensions | `~/.pi/agent/extensions` (root `*.ts` and `*/index.ts` entrypoints); trusted project `.pi/extensions`; packages (`~/.pi/agent/npm` user-scoped, `.pi/npm` project-scoped); CLI `-e` | Native code extension point for tools, commands, UI, and lifecycle events. Extensions run with the Pi process permissions. |
