...
func FormatMarkdown(res *DoomsdayAuditResult) string {
	...
	b.WriteString("| :--- | :---: | :--- | :---: |\n")
	if res.Mode == ModeLean {
		b.WriteString(fmt.Sprintf("| **第一类: 模块契约与影响** | 1 位 (Scout-M) | API 兼容性与 README 承诺综合审核 | `%.2f` |\n", res.ModuleScore))
		b.WriteString(fmt.Sprintf("| **第二类: 裸机工程通用盲审** | 1 位 (Scout-G) | 资源泄漏/并发/单测打假综合盲审 (信息结界) | `%.2f` |\n", res.EngineeringScore))
		b.WriteString(fmt.Sprintf("| **第三类: 宏观目标与终局** | 1 位 (Scout-T) | 核心目标兑现与系统性反噬综合评估 | `%.2f` |\n\n", res.GoalScore))
	} else {
